...
- Store 文件优先使用 `.ts`，状态、getter、action 的输入输出类型显式声明，避免隐式 `any` 扩散。
...
- `AuthState` 等状态类型统一放 `src/types/` 或 store 邻近目录，避免页面层重复定义。
...
- Store 的 action、getter 不声明类型，导致调用方到处手写断言。
...
- 状态模型与 action 返回值是否具备可追踪的 TS 类型声明。
