...
- 输入参数必须校验：`rows > 0`、`cols > 0`、`gap >= 0`，且均为整数。
- App 端需处理相册/文件权限与保存能力差异（Android/iOS 分端验证）。

## 主流程

1. 用户上传单张图片。
2. 用户输入 `rows`、`cols` 与 `gap`。
3. 系统校验输入与图片可用性。
4. 调用切片算法，生成切片数据集合。
5. 页面展示切片预览并提供下载动作。
...
- gap 表示切片间留白像素，先从原图宽高扣除总 gap，再进行网格分配。
- 当图片宽高无法整除行列时，按向下取整切分，剩余像素归并到最后一行/列。
- gap 过大导致可切分宽高不为正时，需提示参数超限并终止切片。
- 批量切片按图片选择顺序串行执行，共用同一组 `rows/cols/gap` 参数。
- 批量切片前需进行像素阈值校验：单张不超过 `24,000,000` 像素，总量不超过 `80,000,000` 像素。
- Canvas 工作区按“当前正在切片的图片尺寸”动态设置，切片完成后释放，避免长期占用最大尺寸内存。
- 批量切片过程中每处理若干张需让出主线程，降低长任务造成的卡顿风险。
...
- H5：导出全部时打包为 `zip` 后一次下载，规避浏览器多文件下载拦截。
- H5：批量导出 zip 目录按 `image-{n}/slice-r{row}-c{col}-{index}.png` 组织。
