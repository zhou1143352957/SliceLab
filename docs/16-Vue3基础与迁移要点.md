...
- JSX/TSX 能力在 uni-app 中仅支持 `App-vue3` 与 `H5-vue3`，迁移方案需避免把 JSX 作为全端统一写法。
- 使用 JSX/TSX 时，`vite` 配置必须启用 `@vitejs/plugin-vue-jsx` 并保持版本与 `vite` 主版本匹配。
...
- JSX 项目在 `vite.config` 中启用 `vue-jsx` 插件后，仅在 H5/App Vue3 端启用对应渲染组件。
...
- 在小程序目标端引入 JSX 组件并作为主渲染链路，导致端能力不兼容。
- 未对齐 `@vitejs/plugin-vue-jsx` 与 `vite` 版本，导致构建报错。
...
- JSX/TSX 使用范围是否限制在 `App-vue3/H5-vue3` 且已验证目标端能力。
- `vite` 与 `@vitejs/plugin-vue-jsx` 版本是否匹配。
