...
- 若项目使用 JSX/TSX，`vite.config` 需启用 `@vitejs/plugin-vue-jsx`，并保持插件版本与 `vite` 主版本一致。
- 若启用 H5 SSR，工程需明确区分“静态资源（client）”与“服务端渲染（server）”两套产物职责。
...
- SSR 场景下 `vite.config` 需明确 `base`（指向前端网页托管地址）与 `ssr.format`（按官方示例使用 `cjs`）。
...
- JSX 配置示例：`plugins: [vue(), vueJsx()]`。
- SSR `main.js` 示例：`createSSRApp` + `app.use(store)`，并在 `createApp` 返回值中显式返回 `store`。
...
- JSX 已启用但 `vite` 与 `@vitejs/plugin-vue-jsx` 版本不匹配，导致构建失败。
- 启用 SSR 但未区分 client/server 产物职责，部署后页面可访问但无法正确激活。
- SSR 使用 Vuex 但 `createApp` 未返回 `store`，导致服务端预取与客户端状态对齐失败。
...
- JSX 项目是否已正确配置 `@vitejs/plugin-vue-jsx` 且版本匹配。
- SSR 工程是否已配置 `base` 与 `ssr.format`，并与部署环境一致。
- `createApp` 返回值是否包含 SSR 所需对象（如 `store`）。
