...
export function validateSliceParams(rows: number, cols: number): void {
...
export function buildSliceRects(
  ...
  const widthRemainder = imageWidth - baseWidth * cols
  const heightRemainder = imageHeight - baseHeight * rows

  if (baseWidth <= 0 || baseHeight <= 0) {
    throw new Error('切片参数超过图片尺寸，请减少行列数后重试')
  }
...
export async function sliceImageByCanvas(
  ...
  const rects = buildSliceRects(imageWidth, imageHeight, rows, cols)
